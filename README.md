# Salutations !
Je m'appelle **Edouard** (Itm0z). Je suis un **jeune développeur** spécialisé dans la création de **petits sites internet**, de **plugins Minecraft**, de **bots Discord** (en **Python**), et plein d'autres choses :)

## Projets récents :
- **Echorys** (de [CrowDEV](https://itm0zlegends.github.io/CrowDev/index.html)) : Un système de sonnerie pour vos serveurs Discord RP (Collège / Lycée)
- **CoreLittle** : Un plugin léger pour Minecraft qui vise à enrichir l'expérience de jeu.
- **ISODOWNLOADER** : Un site web permettant de télécharger facilement et en toute sécurité des images ISO.

## Compétences :

- **Langages** : Java, Python, HTML, CSS

- **Outils** : VSCode, IntelliJ IDEA, MCreator, Godot (pas très souvent)

## Contact : 

- **GitHub** : Itm0zLegends
- **Email** : edouard@mroczek.fr
- **Discord** : @itm0zlegends
